...
def parse_table_rows(path: Path, rows: list[list[str]], target: str) -> TableSchema:
    normalized_rows = [_normalize_row(row) for row in rows]
    if len(normalized_rows) < 3:
        raise ValueError(f"{path} must contain at least 3 metadata rows")

    var_row, desc_row, tag_row = normalized_rows[0], normalized_rows[1], normalized_rows[2]
    ...
    width = max(len(var_row), len(desc_row), len(tag_row))
    var_row = _pad_row(var_row, width)
    desc_row = _pad_row(desc_row, width)
    tag_row = _pad_row(tag_row, width)

    data_rows = [_pad_row(row, width - 1) for row in normalized_rows[3:] if any(cell.strip() for cell in row)]
    columns: list[Column] = []
...
def _normalize_row(row: list[object]) -> list[str]:
    last = -1
